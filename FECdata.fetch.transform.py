...
API_KEY = 'h3pdYZNm4O69cZIeogPkzx0keh7tBqkoPphTVntC' FEC_ENDPOINT = 'https://api.open.fec.gov/v1/schedules/schedule_a/' GRAPH_JSON_PATH = 'graph.json'  # Adjust this path as needed

Base parameters for the FEC API

base_params = { 'api_key': API_KEY, 'contributor_state': 'CO', 'contributor_employer': 'jeffco', 'per_page': 1000 }

Load existing graph data or initialize new structure

if os.path.exists(GRAPH_JSON_PATH) and os.path.getsize(GRAPH_JSON_PATH) > 0: with open(GRAPH_JSON_PATH, 'r') as f: graph = json.load(f) else: graph = {'nodes': [], 'edges': []}

Use dicts for fast lookup to avoid duplicates

existing_node_ids = {node['id'] for node in graph['nodes']} existing_edges = {(e['source'], e['target'], e['label']) for e in graph['edges']}

page = 1 more_pages = True

while more_pages: params = base_params.copy() params['page'] = page response = requests.get(FEC_ENDPOINT, params=params) data = response.json()
...
with open(GRAPH_JSON_PATH, 'w') as f: json.dump(graph, f, indent=2)
